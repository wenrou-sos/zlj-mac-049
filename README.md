...
出入库与移库台账、展陈布撤展、修复过程记录、**馆藏盘点(差异复核与调整审批)**、
**库室温湿度异常自动告警**、**借展到期 / 逾期跟踪**、运行总览看板。
...
| 馆藏盘点 | 按库房/类别/等级发起带截止日期的盘点任务;扫码或人工逐件登记;未盘/盘盈/盘亏/错位/损坏差异记录;复核 → 调整申请 → 审批执行 → 结案归档全程留痕 |
| 环境监测 | 72 小时温湿度双曲线(含阈值标线)、自动告警、确认处理、异常模拟注入 |

### 盘点业务规则

- 同一藏品同一时间只能被一个「进行中」任务占用(建任务与盘盈登记时均校验)
- 扫描范围外藏品自动登记为**盘盈**;账面位置与实物位置不符登记为**错位**
- 差异必须**复核通过**后才能提请调整;调整申请经**审批执行**时才改写正式档案并自动登记「盘点调整」流转台账
- 全部藏品已盘、差异已复核、调整已处理方可结案;结案汇总、盘点人、复核人、审批人全程可溯(明细「痕迹」时间线)
...
- **2 个盘点任务**:1 个已结案(玉石器年度盘点,含错位复核与调整执行全痕迹)+ 1 个进行中(书画库季度盘点,含待复核损坏、待审批盘盈调整)
...
│   │   └── routers/           # dashboard/collections/locations/exhibitions/restorations/loans/environment/inventory
│   └── requirements.txt
└── frontend/
    └── src/
        ├── api/               # axios 接口封装
